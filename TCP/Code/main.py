...
class mywindow(QMainWindow, Ui_Client):
    def __init__(self):
        ...
        self.label_Video.setPixmap(QPixmap('image/Pico_W_4WD_M_Car.png'))
        ...
        self.Rotation.clicked.connect(self.on_btn_Rotate)
        ...
        self.Btn_Move_Left.pressed.connect(self.on_btn_Move_Left)
        self.Btn_Move_Left.released.connect(self.on_btn_Stop)
        self.Btn_Move_Right.pressed.connect(self.on_btn_Move_Right)
        self.Btn_Move_Right.released.connect(self.on_btn_Stop)

        self.Btn_Dia_Left.pressed.connect(self.on_btn_Dia_left)
        self.Btn_Dia_Left.released.connect(self.on_btn_Stop)
        self.Btn_Dia_Right.pressed.connect(self.on_btn_Dia_Right)
        self.Btn_Dia_Right.released.connect(self.on_btn_Stop)
        self.Btn_DiaD_Left.pressed.connect(self.on_btn_DiaD_left)
        self.Btn_DiaD_Left.released.connect(self.on_btn_Stop)
        self.Btn_DiaD_Right.pressed.connect(self.on_btn_DiaD_Right)
        self.Btn_DiaD_Right.released.connect(self.on_btn_Stop)
        self.Btn_Modechange.pressed.connect(self.on_btn_Modechange)
    ...
    def keyPressEvent(self, event):
        ...
        if event.isAutoRepeat():
            ...
            if event.key() == Qt.Key_W:
                ...
                self.on_btn_Move_Left()
                self.Key_A = True
            elif event.key() == Qt.Key_D:
                self.on_btn_Move_Right()
                self.Key_D = True
            elif event.key() == Qt.Key_Space:
                self.on_btn_Buzzer()
                self.Key_Space = True
            elif event.key() == Qt.Key_Q:
                self.on_btn_Dia_left()
                self.Key_Q = True
            elif event.key() == Qt.Key_E:
                self.on_btn_Dia_Right()
                self.Key_E = True
    ...
    def keyReleaseEvent(self, event):
        if (event.key() == Qt.Key_W):
            ...
            if not (event.isAutoRepeat()) and self.Key_X == True:
                self.on_btn_Stop()
                self.Key_X = False
    ...
    def on_btn_ForWard(self):
        if self.commandFlag:
            ...
            M_ForWard = self.intervalChar + str(0) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_ForWard = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                0) + self.intervalChar + str(50) + self.endChar
            if self.ModeFlag:
                if self.RotateFlag:
                    self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_ForWard)  # Rotate mode
                else:
    ...
    def on_btn_Turn_Left(self):
        if self.commandFlag:
            ...
            M_Turn_Left = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                90) + self.intervalChar + str(50) + self.endChar
            if self.ModeFlag:
    ...
    def on_btn_BackWard(self):
        if self.commandFlag:
            ...
            M_BackWard = self.intervalChar + str(180) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_BackWard = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                180) + self.intervalChar + str(50) + self.endChar
            if self.ModeFlag:
                if self.RotateFlag:
                    self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_BackWard)
                else:
    ...
    def on_btn_Move_Left(self):
        if self.commandFlag:
            ...
            self.UltrasonicFlag = 0
            self.Ultrasonic.setText("Ultrasonic off")
            Move_Left = self.intervalChar + str(90) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_Move_Left = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                90) + self.intervalChar + str(50) + self.endChar
            if self.RotateFlag:
                self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_Move_Left)
            else:
                self.TCP.sendData(cmd.CMD_M_MOTOR + Move_Left)

    def on_btn_Move_Right(self):
        if self.commandFlag:
            self.trackFlag = 0
            self.Track.setText("Track off")
            self.lightFlag = 0
            ...
            self.UltrasonicFlag = 0
            self.Ultrasonic.setText("Ultrasonic off")
            Move_Right = self.intervalChar + str(-90) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_Move_Right = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                -90) + self.intervalChar + str(50) + self.endChar
            if self.RotateFlag:
                self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_Move_Right)
            else:
                self.TCP.sendData(cmd.CMD_M_MOTOR + Move_Right)

    def on_btn_Dia_left(self):
        if self.commandFlag:
            self.trackFlag = 0
            self.Track.setText("Track off")
            self.lightFlag = 0
            ...
            self.UltrasonicFlag = 0
            self.Ultrasonic.setText("Ultrasonic off")
            Dia_Left = self.intervalChar + str(45) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_Dia_Left = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                45) + self.intervalChar + str(50) + self.endChar
            if self.RotateFlag:
                self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_Dia_Left)
            else:
                self.TCP.sendData(cmd.CMD_M_MOTOR + Dia_Left)

    def on_btn_Dia_Right(self):
        if self.commandFlag:
            self.trackFlag = 0
            self.Track.setText("Track off")
            self.lightFlag = 0
            ...
            self.UltrasonicFlag = 0
            self.Ultrasonic.setText("Ultrasonic off")
            Dia_Right = self.intervalChar + str(-45) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_Dia_Right = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                -45) + self.intervalChar + str(50) + self.endChar
            if self.RotateFlag:
                self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_Dia_Right)
            else:
                self.TCP.sendData(cmd.CMD_M_MOTOR + Dia_Right)
    ...
    def on_btn_DiaD_left(self):
        if self.commandFlag:
            ...
            self.UltrasonicFlag = 0
            self.Ultrasonic.setText("Ultrasonic off")
            DiaD_Left = self.intervalChar + str(135) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_DiaD_Left = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                135) + self.intervalChar + str(50) + self.endChar
            if self.RotateFlag:
                self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_DiaD_Left)
            else:
                self.TCP.sendData(cmd.CMD_M_MOTOR + DiaD_Left)
    ...
    def on_btn_DiaD_Right(self):
        if self.commandFlag:
            ...
            self.UltrasonicFlag = 0
            self.Ultrasonic.setText("Ultrasonic off")
            DiaD_Right = self.intervalChar + str(-135) + self.intervalChar + str(50) + self.intervalChar + str(
                0) + self.intervalChar + str(0) + self.endChar
            R_DiaD_Right = self.intervalChar + str(0) + self.intervalChar + str(0) + self.intervalChar + str(
                -135) + self.intervalChar + str(50) + self.endChar
            if self.RotateFlag:
                self.TCP.sendData(cmd.CAR_CAR_ROTATE + R_DiaD_Right)
            else:
                self.TCP.sendData(cmd.CMD_M_MOTOR + DiaD_Right)
    ...
    def on_btn_Stop(self):
        if self.commandFlag:
            ...
            if self.ModeFlag:
                if self.RotateFlag:
                    self.TCP.sendData(cmd.CAR_CAR_ROTATE + M_Stop)
                else:
    ...
    def on_btn_Modechange(self):
        if self.ModeFlag:
            self.Btn_Move_Left.hide()
            self.Btn_Move_Right.hide()
            self.Btn_Dia_Left.hide()
            self.Btn_Dia_Right.hide()
            self.Btn_DiaD_Left.hide()
            self.Btn_DiaD_Right.hide()
            self.Rotation.hide()
            self.Btn_Turn_Left.move(85, 460)
            self.Btn_Turn_Right.move(260, 460)
            self.label_Video.setPixmap(QPixmap('image/Pico_W_4WD_Car.png'))
            self.ModeFlag = 0
        else:
            self.Btn_Move_Left.show()
            self.Btn_Move_Right.show()
            self.Btn_Dia_Left.show()
            self.Btn_Dia_Right.show()
            self.Btn_DiaD_Left.show()
            self.Btn_DiaD_Right.show()
            self.Rotation.show()
            self.Btn_Turn_Left.move(10, 460)
            self.Btn_Turn_Right.move(350, 460)
            self.label_Video.setPixmap(QPixmap('image/Pico_W_4WD_M_Car.png'))
            self.ModeFlag = 1
    ...
    def on_btn_Track(self):  # Tracking car control
        if self.commandFlag:
            if self.trackFlag == 0:
                ...
                self.Ultrasonic.setText("Ultrasonic off")
                self.Light.setText("Light off")
                self.Rotation.setText("Rotate off")
    ...
    def on_btn_Rotate(self):  # Rotate car control
        if self.commandFlag:
            if self.RotateFlag == 0:
                ...
                self.Track.setText("Track off")
                self.Light.setText("Light off")
                self.TCP.sendData(cmd.CMD_CAR_MODE + self.intervalChar + '4' + self.endChar)
                self.Rotation.setText("Rotate on")
            else:
                self.RotateFlag = 0
                self.TCP.sendData(cmd.CMD_CAR_MODE + self.intervalChar + '0' + self.endChar)
                self.Rotation.setText("Rotate off")
